...
**触发条件源码**（`tools/skill_manager_tool.py` 的 `SKILL_MANAGE_SCHEMA` description）：
...
**阈值确认**：默认触发阈值是 **10 次工具调用**，配置路径为 `skills.creation_nudge_interval`。

**触发条件不是单一因素**，以下任一情况都会触发：

1. **工具调用累积**：完成复杂任务（≥10次工具调用）
2. **克服了错误**：在错误中找到了正确方法
3. **用户纠正有效**：用户纠正后方法被验证有效
4. **发现非平凡工作流**：找到了值得复用的工作模式
5. **用户主动要求**：用户明确说"记住这个流程"
...
> - 解决办法是**关掉 `creation_nudge_interval`（设为 0 或者较大的值）+ 每周 review 一次**，不用的 Skill 直接删。
